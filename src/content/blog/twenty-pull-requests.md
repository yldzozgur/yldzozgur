...
A real codebase already has thousands of decisions baked in. The first thing I had to do at Ameza was spend a week just *reading* — following imports, tracing how a request flowed from the React form through the validation utilities through the typed DTO through the controller through the service through the database. There was no bug to fix yet. I was just trying to load the map into my head.
...
- **Described.** Every PR needs a description that explains *why*, not *what*. The diff already shows what.
...
When you're alone, the best code is the code you understand. When you're on a team, the best code is the code *everyone* understands. That means following the existing pattern even when you'd write it differently from scratch.
...
Every review on my PRs has taught me something I would never have discovered on my own: an existing utility I didn't know about, a TypeScript pattern I hadn't seen, a domain decision I had quietly violated. The first comment on my first PR was: *"We use the `discriminatedUnion` pattern for these — see `src/errors/codes.ts`."* I didn't even know discriminated unions existed.
