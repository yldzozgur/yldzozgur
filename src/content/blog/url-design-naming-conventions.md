...
GET /users/:id    → single item from the same collection
```

If you used singular (`/user`), you'd end up with `/user/:id` for the item and `/users` for the collection — an inconsistency that forces developers to memorize which form each endpoint uses.
...
URLs are case-sensitive on most servers, but browsers and clients normalize them inconsistently. Sticking to lowercase eliminates case-sensitivity bugs entirely. Use hyphens instead of underscores — underscores can be hidden under link underlines and are harder to type correctly.
...
GET  /users/:userId/posts/:postId  → specific post by a specific user
```

Keep nesting shallow — two levels max. Deep nesting creates long, hard-to-read URLs:
...
```
# Wrong — these are not separate resources
GET /posts/published
GET /posts/page/2
...
Sometimes you need endpoints for operations that don't fit the resource model cleanly. Activating an account, sending an email, archiving a record — these are actions.
...
```
POST /users/:id/activation     → activate user
DELETE /users/:id/activation   → deactivate user
```
